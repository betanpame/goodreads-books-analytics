"""Portfolio 01 - repeatable initial inspection CLI for Goodreads books.

The script is the first stop in the portfolio spine: it recreates the
hands-on walkthrough from Phase 01 but in a Docker-friendly CLI that verifies
the environment, fabricates a tiny CSV demo, and emits lightweight artifacts
(`outputs/initial_inspection/`). Recruiters see this as proof that the repo is
ready to explore before the heavier notebooks and SQL demos kick in.
"""
