"""EDA entry point for the Goodreads books dataset.

The script focuses on four responsibilities:
1. Configure plotting libraries (matplotlib + seaborn) with consistent styling.
2. Load the Phase 02 output dataset so later tasks can build on the same DataFrame.
3. Produce univariate summaries (numeric, categorical, outlier scaffolding).
4. Generate core bivariate visuals that connect ratings with book size/popularity.
"""
