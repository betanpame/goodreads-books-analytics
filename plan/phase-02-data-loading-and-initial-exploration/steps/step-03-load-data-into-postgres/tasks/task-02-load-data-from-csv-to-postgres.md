...
1. Choose a loading approach:
   ...
   - **Option B – Python**: Use a analysis script or script with `pandas` and `sqlalchemy` to insert data.
2. Document your chosen approach in a markdown file or in your analysis script.
3. Implement the loading process:
   - For `COPY`: write a SQL script (e.g., `sql/load_books_from_csv.sql`).
   - For Python: write a small script or analysis script section that reads the CSV and writes to PostgreSQL.
...
- [ ] I have implemented the loading process in a script or analysis script.
